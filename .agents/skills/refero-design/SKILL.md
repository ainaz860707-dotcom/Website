---
name: refero-design
description: "Исследование дизайна по живым продуктам: 150k+ экранов и флоу Refero плюс встроенные справочники по типографике, цвету, ритму, движению, иконкам и копирайту. Вызывай на «посмотри как сделано у живых продуктов», «найди референсы экранов», «как выглядят дашборды в этой нише», «разбери визуальный язык», «покажи флоу онбординга», «страница выглядит как AI-поделка», а также как источник примеров внутри niche-benchmark и перед вёрсткой в design-taste-frontend. Design research skill: styles (visual direction), screens (concrete UI patterns), flows (journeys), plus bundled craft knowledge on typography, color, spacing, motion, icons, copywriting, anti-AI-slop. Works without the Refero MCP (bundled references); live search requires the MCP connection. ПОДЧИНЁН маршруту проекта, а не заменяет его: числа лестниц берутся из design-system-source, движение — из motion-direction, разведка ниши — из niche-benchmark, вёрстка — из design-taste-frontend; этот скилл даёт им доказательства и примеры. §12 CLAUDE.md сильнее: ни одного текста, цены, отзыва или факта с найденных референсов на страницу клиента не переносится."
---

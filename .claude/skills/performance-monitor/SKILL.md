...
> Emit the standard shape from [skill-contract.md §Handoff Summary Format](../../seo-geo-shared/references/skill-contract.md). Name the active mode (`report` / `alert`) in the Objective line.

## Data Sources

All integrations optional (see [CONNECTORS.md](../../seo-geo-shared/CONNECTORS.md)). Tier 1 (keyless) works for both modes; keyed tools are opt-in Tier 2/3.
...
**Zero-dependency measurement loop** (both modes): every reported change or fired alert should come from a computed delta, not an eyeballed estimate. Store each period's KPIs and let the ledger compute movement: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/connectors/ledger.py" record <domain> --source monitor --data '{"sessions": ..., "clicks": ..., ...}'`, then `ledger.py diff <domain> --source monitor` for the period delta and `ledger.py trend <domain> --source monitor --field <kpi>` for the trend line. Label every figure Measured / User-provided / Estimated, and attribute outcome movement against a control rather than to the latest change — see [references/measurement-protocol.md](../../seo-geo-shared/references/measurement-protocol.md). See [scripts/connectors/README.md](../../seo-geo-shared/scripts/connectors/README.md).
...
Ask "Save these results?" If yes, write to `memory/monitoring/` using filename `YYYY-MM-DD-<topic>.md` — see [skill-contract.md §Save Results Template](../../seo-geo-shared/references/skill-contract.md). This is a non-auditor skill: ask before writing memory and hand off veto-like risks to the relevant auditor gate rather than appending veto markers itself.
...
- [Measurement & Attribution Protocol](../../seo-geo-shared/references/measurement-protocol.md) — readback windows, required fields, and the promote / keep-testing / rollback / unproven decision rule

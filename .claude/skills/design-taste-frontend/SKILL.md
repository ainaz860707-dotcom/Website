...
description: Anti-slop frontend skill for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check. Вызывай на «сделай лендинг», «свёрстай страницу», «выглядит как шаблон», «выглядит как AI-поделка», «убери типовщину», «подбери дизайн-направление», «портфолио», а также перед вёрсткой любой промо-страницы. Не для дашбордов и таблиц данных. Числа лестниц берутся из design-system-source, движение — из motion-direction.
---
...
### 0.A2 Project rule that overrides this skill — no invented facts

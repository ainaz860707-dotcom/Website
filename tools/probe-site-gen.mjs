...
- ПОДПИСИ НЕЙТРАЛЬНЫЕ. Нельзя писать «наш цех», «наша пасека», «наши работы», «фото
  ...
  Притяжательное местоимение — не единственный способ соврать. Оборот «где проходят
  консультации», «здесь принимают», «в этом зале работает мастер» привязывает чужой кадр
  к бизнесу клиента ровно так же и запрещён наравне с «наш». Проверено на живой странице
  психолога: alt «Рабочее место, где проходят консультации» стоял у стокового снимка,
  снятого неизвестно где. Alt описывает ТОЛЬКО то, что физически видно в кадре, без
  привязки к этому делу, этому адресу и этому человеку.
  И обратное: если в кадре не то, о чём страница (интерьер галереи на странице психолога),
  снимок не берётся вовсе — правило «не подходит по смыслу — не бери» сильнее желания
  заполнить блок.
...
1. Базовый цвет из палитры. Он НЕ чисто-белый: даже в «плоских» дирекшенах база уведена
   на полтона в тепло или в холод, чтобы белым читались только карточки поверх неё.
2. Один-два очень широких радиальных градиента силой 2–6%, цветом из палитры — это свет,
   падающий на материал. Границ у них нет, каждый занимает от половины экрана.
3. Зерно — если дирекшен его описал. Файлов у страницы нет, поэтому зерно делается
   инлайновым SVG-шумом в data:-URI, а не картинкой по ссылке:
   ...
   Крупность зерна задаётся baseFrequency: 0.45 — грубое, видимое; 0.9 — тонкое, заметное
   только вблизи. Сила — opacity от 0.02 до 0.14 по описанию дирекшена, не больше.
Зерно вешается на неподвижный слой поверх страницы — отдельный ::before у body с
position:fixed, inset:0, pointer-events:none — либо прямо в background у body. На
прокручиваемые контейнеры и на карточки зерно не вешается: браузер перерисовывает шум
на каждом кадре прокрутки, и на телефоне страница начинает дёргаться.
Контраст текста считается от базового цвета и остаётся не ниже 4.5:1 — слои света и зерна
не имеют права опустить его ниже. Фон не движется и не дышит, если этого не просит пресет
движения; при prefers-reduced-motion он неподвижен всегда.
Тёмные секции внутри светлой страницы наследуют тот же материал в тёмном тоне, а не
становятся плоской чёрной заливкой: страница сделана из одного вещества сверху донизу.
...
- Меню в шапке на узком экране: либо оно целиком уезжает в отдельный ряд под названием и
  кнопкой, либо его там нет вовсе. Ужимать ряд ссылок между названием и кнопкой запрещено:
  на 390px от второго пункта остаётся вертикальный огрызок, который читается как случайная
  палка. Проверяется так: на 390px у каждой видимой ссылки меню ширина не меньше её текста.
- Метка секции ОДНА. Порядковый номер и надзаголовок-категория вместе над одним и тем же
  заголовком — двойная типовая метка, её видно как трафарет: выбирай что-то одно.

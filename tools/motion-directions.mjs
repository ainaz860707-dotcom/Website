...
export const TECHNIQUES = [
  ...
  },
  {
    ...
    rule: `Продукт нарисован крупно и не стоит на месте: мёд льётся струёй и наполняет банку,
  кофе наливается слоями и молоко вмешивается спиралью, тесто поднимается, краска
  растекается. Продукт — инлайновый SVG, фотографий нет.

  МЕХАНИКА НАПОЛНЕНИЯ. Уровень жидкости поднимается через clip-path или mask поверх формы,
  а НЕ через height — иначе форма растягивается вместо наполнения. Струя — path со
  stroke-dasharray и бегущим stroke-dashoffset, с лёгким изгибом, а не прямой палкой.
  Поверхность жидкости качается собственным циклом 2–3s. Капли и брызги — отдельные
  элементы с разными задержками, они падают по дуге и исчезают, а не мигают на месте.

  СЦЕНА ВЕДЁТСЯ СКРОЛЛОМ. Секция закрепляется на position: sticky, продукт в центре
  проходит 3–4 стадии по прогрессу скролла (пустая банка → льётся → полная → закрыта),
  и ВМЕСТЕ с ним меняется заголовок рядом: своя короткая фраза на каждую стадию. Тексты
  всех стадий лежат в разметке подряд и читаются без прокрутки — меняется только видимость
  под .js. Это тот самый приём из продуктовых лендингов: смысл несёт не картинка, а связка
  «стадия продукта + фраза о нём».

  РОЙ ВОКРУГ ПРОДУКТА. 5–9 мелких объектов ниши — пчёлы у мёда, зёрна у кофе, лепестки у
  цветов, снежинки у мороженого — летают по РАЗНЫМ кривым: каждому свой offset-path,
  offset-distance от 0% до 100%, длительность 6–14s, задержка своя, направление у части
  обратное. Мелкая деталь трепещет отдельным быстрым циклом: крылья 0.12–0.2s, пар 3s.
  Одинаковые траектории и синхронный полёт — брак, это читается как гирлянда.
  На ширине до 640px рой сокращается до трёх объектов.

...
export const TECHNIQUES = [
  ...
  «до/после» — выдуманный факт о бизнесе и брак. Сгенерированная моделью «грязная вещь»
  под той же подписью — тот же брак. Нет пары снимков — приём всё равно ставится, но обоими
  кадрами идут слоты-плейсхолдеры с подписью «кадр мастерской» и описанием нужного снимка:
  механизм виден и работает, а места под фото остаются пустыми до пары от клиента.
...
animation-timeline: none — БРАК, ХУДШИЙ ИЗ ВОЗМОЖНЫХ. Значение означает «таймлайна нет»,
анимация не продвигается никогда и с animation-fill-mode: both навсегда застревает на
кадре 0%. Элемент с opacity:0 в этом кадре невидим у КАЖДОГО посетителя, а не только без
скриптов; вёрстка и сборка при этом зелёные. Проверено на живой странице: пять элементов
первого экрана — H1, подводка, кнопки, строка фактов, надзаголовок — не показались вообще.
Отменять унаследованный view() значением none запрещено. Обычной временной анимации
свойство animation-timeline не пишется ВОВСЕ: нет свойства — есть время документа.
